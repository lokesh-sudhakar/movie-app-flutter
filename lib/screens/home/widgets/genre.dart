    ...
    return BaseView<GenreViewModel> (
      initState: (model) {
        model.getGenres().then((value) {
          _tabController = TabController(initialIndex: 0,length: value.genres.length,vsync: this);
        });
      },
      builder: (context, model, child) => Container(
        ...
        child: Center(
          child: model.state == ViewState.Busy?  CircularProgressIndicator() :
              model.genreResponse.error.isNotEmpty ? CustomErrorWidget(model.genreResponse.error) :
          Container(
